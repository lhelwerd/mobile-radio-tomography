...
class WaypointsTableWidget(QtGui.QTableWidget):
    ...
    def set_row_data(self, row, data):
        ...
        The given `row` must be a row index number, and this row must already
        exist. If the `data` is not long enough, i.e., it does not provide
        ...
        for col, column in enumerate(self._columns):
            ...
            if "widget" in column:
                self.cellWidget(row, col).set_value(data[col])
